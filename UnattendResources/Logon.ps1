...
$PSWindowsUpdateDir = (Join-Path $resourcesDir "PSWindowsUpdate")
...
try
{
    ...
    if($installUpdates -and (Test-Path "$resourcesDir\PSWindowsUpdate"))
    {
        $Host.UI.RawUI.WindowTitle = "Installing updates..."

        Import-Module "$resourcesDir\PSWindowsUpdate"

        Get-WUInstall -AcceptAll -IgnoreReboot -IgnoreUserInput -UpdatesLimit 100
